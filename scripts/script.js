...
fetch('/data/data.json').then(res => res.json()).then(data => {
    if (federalPage) {
        ...
        document.getElementById('level').innerText = 'State governments';
        data = data.filter(domain => domain.name.startsWith('State Of'));
        filterInput.value = 3;
    }
    else if (agencyPage) {
        ...
        document.getElementById('level').innerText = 'Metadata';
    }
