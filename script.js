...
const displayAgencies = (agencies, filter) => {
    ...
    for (let i = 100 * (pageNumber - 1); i < Math.min(100 * pageNumber, data.length); i++) {
        ...
        tr.innerHTML = `
                ...
                <td class="text-center"><span class="badge text-bg-${agency.status !== 200 ? '' : (percent >= 0.9 ? 'success' : (percent >= 0.7 ? 'warning' : 'danger'))} font-weight-normal bg-opacity-75" title="${percent * 100}%">${agency.status == 200 ? (percent >= 0.9 ? 'A' : percent >= 0.8 ? 'B' : percent >= 0.7 ? 'C' : percent >= 0.6 ? 'D' : 'F') : '－'}</span></td>
